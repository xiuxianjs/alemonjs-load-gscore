...
import { deliver } from "./gscore/delivery.js";
...
const responseRouter = defineRouter([
	...
	},
	{
		regular: /^\/我是主人\s*$/,
		selects: ["message.create", "private.message.create"],
		handler: lazy(() => import("./response/owner-claim.js"))
	},
	{
		regular: /^(?!(?:\/|#|＃)gs\b).*/i,
		selects: ["message.create", "private.message.create"],
	...
	onCreated() {
		(async () => {
			if (getRuntimeMode() === "local" && getAutoStart()) try {
				...
				if (status.installed && !status.running) await manager.start();
				...
				logger.info(running ? `[alemonjs-load-gscore] 已连接 GsCore ${manager.transport === "websocket" ? "WebSocket" : "HTTP"} 服务` : "[alemonjs-load-gscore] GsCore 未连接；请执行 #gs安装 或检查 GsCore 配置");
			}
		})();
	}
});
