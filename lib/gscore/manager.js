import { getApiToken, getAutoStart, getBotID, getContainerName, getDataDir, getDockerImage, getGSCoreCoreDir, getGSCoreDir, getGSCoreLogsDir, getGSCoreRepo, getGSCoreURL, getGSCoreVenvDir, getGSCoreVenvPython, getMessageTimeout, getPluginsDir, getPort, getPythonCommand, getRuntimeMode, getStartupTimeout, getTransport, getWSToken } from "../path.js";
import { GSCoreWebSocketAdapter } from "./adapter.js";
...
var GSCoreManager = class GSCoreManager {
	...
	writeBridgeConfig(enableHTTP = getTransport() === "http") {
	...
	async startLocalProcess() {
		...
		if (await this.serviceReachable()) throw new Error("GsCore 已在配置地址运行，当前进程不是本插件启动的；请勿重复启动，或先切换为 external 模式");
	...
	async refresh(force = false) {
		if (!force && Date.now() - this.lastRefreshAt < GSCoreManager.REFRESH_INTERVAL) return this.isReady;
		if (this.refreshTask) return this.refreshTask;
		this.refreshTask = (async () => {
			try {
				if (getTransport() === "websocket") this.reachable = await this.adapter.connect();
				else {
					const response = await request("/app");
	...
	async status() {
		...
		const processRunning = mode === "local" ? Boolean(managedPID) || transportReady || await this.serviceReachable() : transportReady;
		...
		return {
			...
			plugins,
			message: ready ? `GsCore 已通过 ${transport === "websocket" ? "WebSocket" : "HTTP"} 连接。` : processRunning ? "GsCore 进程正在启动，桥接尚未就绪。" : installed ? "GsCore 未运行。" : "GsCore 尚未安装。",
	...
	getCoreCommandPrefix() {
		...
		try {
			const config = JSON.parse(readFileSync(path, "utf8"));
			const forcePrefixes = Array.isArray(config.force_prefix) ? config.force_prefix.map((value) => String(value).trim()).filter(Boolean) : [];
			const required = config.disable_force_prefix !== true && forcePrefixes.length > 0;
			return {
				available: true,
				prefix: required ? forcePrefixes[0] : "",
				required
			};
		} catch (error) {
	...
	async restartAfterConfigChange() {
	...
	async addMaster(userId) {
		...
		const masters = Array.isArray(config.masters) ? config.masters.map((value) => String(value).trim()).filter(Boolean) : [];
