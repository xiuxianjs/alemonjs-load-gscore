import { getApiToken, getGSCoreURL } from "./path.js";
...
async function proxyConsole(ctx, targetPath) {
	...
	try {
		...
		if (upstreamContentType?.includes("text/html") || upstreamContentType?.includes("javascript") || upstreamContentType?.includes("text/css")) ctx.body = data.toString("utf8").replaceAll("/app/", "__GSCORE_APP_PATH__").replaceAll("/api/", "__GSCORE_API_PATH__").replaceAll("__GSCORE_APP_PATH__", "./").replaceAll("__GSCORE_API_PATH__", "../console-api/");
		else ctx.body = data;
	} catch (error) {
		ctx.status = 502;
		ctx.body = {
